...
class iATG:
    ...
    def calculate_attractor_landscapes_for_each_IC(self, use_all_initials=True,
        ...
        if use_all_initials:
            ...
            self.calculate_attractor_landscape_for_each_IC_using_random_initials(waiting_num, difference_threshold, verbose)
            pass
    ...
    def calculate_asynchro_att_landscapes_for_each_IC(self,
        ...
        """Compute the attractor landscapes generated by 
        asynchronous-update state transitions 
        for both IC_basal and IC_transition.

        The parameters `use_all_initials`, `waiting_num`, 
        and `difference_threshold` have the same meanings 
        as in the synchronous-update case.

        `repeat_for_each_state` specifies the number of trajectory 
        simulations performed from each individual initial state.

        Repeated simulations are necessary to account 
        for the randomness inherent in asynchronous updates.

        During trajectory simulation, 
        if a previously visited state is encountered again, 
        the algorithm checks whether a complex attractor 
        containing that state exists."""
        if use_all_initials:
            if use_time_reversal_method:
                print("Calculating asynchronous update attractor landscapes using all initial states and time-reversal method.")
                self.calculate_asynchro_att_landscapes_for_each_IC_using_time_reversal(repeat_for_each_state)
            else:
    ...
    def calculate_asynchro_att_landscapes_for_each_IC_using_all_initials(self, repeat_for_each_state=50):
        ...
        self.attractor_landscape_basal = self.calculate_asynchro_att_landscape_for_specific_IC_using_all_initials(self.IC_basal, repeat_for_each_state)
        print("\nCalculating asynchronous attractor landscape on input configuration {} using all initial states.".format(self.IC_transition))
        self.attractor_landscape_transition = self.calculate_asynchro_att_landscape_for_specific_IC_using_all_initials(self.IC_transition, repeat_for_each_state)
    
    def calculate_asynchro_att_landscapes_for_each_IC_using_time_reversal(self, repeat_for_each_state=50):
        """Calculate the attractor landscape using asynchronous updates 
        for IC_basal and IC_transition 
        using all network state values through time-reversal method."""
        print("\nCalculating asynchronous attractor landscape on input configuration {} through time-reversal method.".format(self.IC_basal))
        self.attractor_landscape_basal = self.calculate_asynchro_att_landscape_for_specific_IC_through_time_reversal(self.IC_basal, repeat_for_each_state)
        print("\nCalculating asynchronous attractor landscape on input configuration {} through time-reversal method.".format(self.IC_transition))
        self.attractor_landscape_transition = self.calculate_asynchro_att_landscape_for_specific_IC_through_time_reversal(self.IC_transition, repeat_for_each_state)
    ...
    def calculate_asynchro_att_landscape_for_each_IC_using_random_initials(self, repeat_for_each_state=50, 
        ...
        self.attractor_landscape_basal = self.calculate_asynchro_att_landscape_for_specific_IC_using_random_initials(self.IC_basal, waiting_num, difference_threshold, verbose, repeat_for_each_state)
        ...
        self.attractor_landscape_transition = self.calculate_asynchro_att_landscape_for_specific_IC_using_random_initials(self.IC_transition, waiting_num, difference_threshold, verbose, repeat_for_each_state)
    ...
    def set_empty_asynchro_att_landscape_for_each_IC_wo_calculation(self, repeat_for_each_state=50):
        """this function is used for reversing control"""
        self.attractor_landscape_basal = Asynchro_att_landscape_for_specific_IC(self.dynamics_Boolean_net, self.IC_basal, self.fixed_node_state_map, repeat_for_each_state)
        self.attractor_landscape_transition = Asynchro_att_landscape_for_specific_IC(self.dynamics_Boolean_net, self.IC_transition, self.fixed_node_state_map,repeat_for_each_state)
    ...
    def calculate_asynchro_att_landscape_for_specific_IC_using_all_initials(self, IC:dict, repeat_for_each_state=50):
        """Calculate the asynchronous attractor landscape for a specific input configuration (IC)."""
        attractor_landscape_for_IC = Asynchro_att_landscape_for_specific_IC(self.dynamics_Boolean_net, IC, self.fixed_node_state_map, repeat_for_each_state)
        attractor_landscape_for_IC.calculate_asynchro_att_basinratios_from_all_initial_states()

        return attractor_landscape_for_IC

    def calculate_asynchro_att_landscape_for_specific_IC_through_time_reversal(self, IC:dict, repeat_for_each_state=50):
        """Calculate the asynchronous attractor landscape for a specific input configuration (IC).
        through time-reversial method"""
        attractor_landscape_for_IC = Asynchro_att_landscape_for_specific_IC(self.dynamics_Boolean_net, IC, self.fixed_node_state_map, repeat_for_each_state)
        attractor_landscape_for_IC.calculate_asynchro_atts_through_time_reversal()
    ...
    def calculate_asynchro_att_landscape_for_specific_IC_using_random_initials(self, IC:dict, waiting_num, difference_threshold, verbose, repeat_for_each_state=50):
        """Calculate the asynchronous attractor landscape for a specific input configuration (IC)."""
        attractor_landscape_for_IC = Asynchro_att_landscape_for_specific_IC(self.dynamics_Boolean_net, IC, self.fixed_node_state_map, repeat_for_each_state)
    ...
    def get_asynchro_att_transitions_induced_by_IC_change_and_calculate_TPs(self, starting_att_code=None):
        """Get the attractor transitions induced by the IC changes.
        Then, calculate the transition probabilities.

        if `starting_att_code` is None, calculate all attractor transitions induced by the IC changes
        if `starting_att_code` is specific att_code, 
        calculate attractor transition induced by IC changes
        which can be path starting from the specific att_code.

        This function is used when the asynchronous update attractor landscape is calculated."""
        if starting_att_code is None:
            #initialize
            self.attractor_transitions_induced_by_IC_change = []
            self.TPs = {} # transition probabilities
            # this is calculated by using the frequency of random transitions
            ...
            while True:
                ...
                self._get_asynchro_att_transitions_induced_by_IC_change_and_calculate_TPs(self.attractor_landscape_basal)
                self._get_asynchro_att_transitions_induced_by_IC_change_and_calculate_TPs(self.attractor_landscape_transition)
            ...
            # not initialize
            # just add new attractor transitions
            # and calculate TPs for new attractor transitions
            queue_to_calculate = []
            if starting_att_code not in self.TPs:
                queue_to_calculate.append(starting_att_code)
            # weather att_code is in self.TPs is used 
            # to check whether attractor transitions 
            # starting from that att_code is already calculated or not.

            while queue_to_calculate:
                # print("queue_to_calculate ", queue_to_calculate)
                att_code_to_calculate = queue_to_calculate.pop()
                # print("calculating for att_code ", att_code_to_calculate)
                ATs_starting_from_att_code, TPs_starting_from_att_code = self._get_asynchro_AT_and_calculate_TPs_starting_from(att_code_to_calculate)
                # print("ATs_starting_from_att_code ", ATs_starting_from_att_code)
                # print("TPs_starting_from_att_code ", TPs_starting_from_att_code)

                for AT_starting_from_att_code in ATs_starting_from_att_code:
                    self.attractor_transitions_induced_by_IC_change.append(AT_starting_from_att_code)
                    att_target_code = AT_starting_from_att_code[1]
                    if att_target_code not in self.TPs:
                        queue_to_calculate.append(att_target_code)

                self.TPs[att_code_to_calculate] = TPs_starting_from_att_code
    ...
    def _get_asynchro_att_transitions_induced_by_IC_change_and_calculate_TPs(self, attractor_landscape_to_be_source):
        """Get the attractor transitions induced by the IC changes and asynchronous update.
        Then, calculate the transition probabilities.
        only consider attractor transitions whose source attractor is in attractor_landscape_to_be_source."""
        ...
        for source_attractor_index in attractor_landscape_to_be_source.attractor_index_map.keys():
            ...
            for asynchro_att_transition in asynchro_att_transitions_induced_by_IC_change_from_the_index:
                ...
                IC_target_att_index = asynchro_att_transition[1]
                attr_target_tuple_form = (IC_target, IC_target_att_index[1])

                asynchro_att_transition_hashable = (attr_source_tuple_form, attr_target_tuple_form)
                if asynchro_att_transition_hashable not in self.attractor_transitions_induced_by_IC_change:
                    self.attractor_transitions_induced_by_IC_change.append(asynchro_att_transition_hashable)
                asynchro_att_transition_count_from_source_attractor[attr_target_tuple_form] = asynchro_att_transition_count_from_source_attractor.get(attr_target_tuple_form,0) +1

            TPs_from_source_attractor = {attr_target_tuple_form: count/len(asynchro_att_transitions_induced_by_IC_change_from_the_index)
                                                              for attr_target_tuple_form, count in asynchro_att_transition_count_from_source_attractor.items()}
            self.TPs[attr_source_tuple_form] = TPs_from_source_attractor
    
    def _get_asynchro_AT_and_calculate_TPs_starting_from(self, att_code):
        ATs_starting_from_att_code = []
        TPs_starting_from_att_code = {}

        if att_code[0] == "basal":
            IC_of_att = self.IC_basal
            IC_target = "transition"
        elif att_code[0] == "transition":
            IC_of_att = self.IC_transition
            IC_target = "basal"
        else:
            raise ValueError("Invalid att_code: {}".format(att_code))
        att_index = att_code[1]

        asynchro_att_transitions_induced_by_IC_change_from_the_index = self.get_asynchro_target_att_for_given_source_attractor_induced_by_IC_change(att_index, IC_of_att)
        asynchro_att_transition_count_from_source_attractor = {}
        for asynchro_att_transition in asynchro_att_transitions_induced_by_IC_change_from_the_index:
            IC_target_att_index = asynchro_att_transition[1]
            attr_target_tuple_form = (IC_target, IC_target_att_index[1])

            asynchro_att_transition_hashable = (att_code, attr_target_tuple_form)
            if asynchro_att_transition_hashable not in ATs_starting_from_att_code:
                ATs_starting_from_att_code.append(asynchro_att_transition_hashable)
                # duplication proof
            asynchro_att_transition_count_from_source_attractor[attr_target_tuple_form] = asynchro_att_transition_count_from_source_attractor.get(attr_target_tuple_form,0) +1

        TPs_starting_from_att_code = {attr_target_tuple_form: count/len(asynchro_att_transitions_induced_by_IC_change_from_the_index)
                                                            for attr_target_tuple_form, count in asynchro_att_transition_count_from_source_attractor.items()}
        return ATs_starting_from_att_code, TPs_starting_from_att_code
    ...
    def get_asynchro_target_att_for_given_source_attractor_induced_by_IC_change(self, source_attractor_index, IC_of_source):
        """Given a source attractor, find the target attractor reached through asynchronous update 
        that is induced by the IC change from IC_of_source to IC_of_target.
        ...
        contain duplicate attractor_transition_induced_by_IC_change.
        These duplicate entries will be used later when calculating the transition probability."""
        asynchro_att_transitions_induced_by_IC_change = []
        ...
        for attractor_state in attractor.attractor_states:
            attractor_state_IC_changed = attractor_state.apply_perturbation(IC_of_target, True)
            for _ in range(attractor_landscape_of_source.repeat_for_each_state):
                target_attractor, target_attractor_index = attractor_landscape_of_target.asynchro_converge_network_state_to_attractor(attractor_state_IC_changed)

                asynchro_att_transition_induced_by_IC_change = ((IC_of_source, source_attractor_index),(IC_of_target, target_attractor_index))
                asynchro_att_transitions_induced_by_IC_change.append(asynchro_att_transition_induced_by_IC_change)
        
        return asynchro_att_transitions_induced_by_IC_change
